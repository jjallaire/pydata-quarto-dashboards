...
::: incremental
1)  **Navigation Bar and Sidebar** --- Icon, title, and author along with links to sub-pages (if more than one page is defined). Sidebars are used to present inputs within interactive dashboards.
...
3)  **Cards (Plots, Tables, Value Boxes, Text)** --- Cards are containers for cell outputs and free form markdown text. The content of cards typically maps to *cells* in your notebook or source document.
:::

::: fragment
*All of these components can be authored and customized within notebook UI*
:::
...
|               |                                                                                          |
|-----------------------|-------------------------------------------------|
| Static        | [Rendered a single time (e.g. when underlying data won't ever change)]{.fragment}        |
| Scheduled     | [Rendered on a schedule (e.g. via `cron` job) to accommodate changing data.]{.fragment}  |
| Parameterized | [Variations of static or scheduled dashboards based on parameters.]{.fragment}           |
| Interactive   | [Fully interactive dashboard using Shiny (requires a server for deployment).]{.fragment} |

: {tbl-colwidths=\[35,65\]}

## ![](images/icon-under-the-hood.svg){.title-icon}
